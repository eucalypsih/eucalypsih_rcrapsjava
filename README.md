...
```
git clone -q --filter=blob:none --sparse git@github.com:eucalypsih/eucalypsih_rcrapsjava.git && cd eucalypsih_rcrapsjava && un="eucalypsih";ue="eucalypsih@gmail.com";git checkout main && git config user.name "$un" && git config user.email "$ue" && git config gpg.format ssh && git config user.signingkey ~/.ssh/id_rsa.pub && git config commit.gpgsign true && git config gpg.ssh.allowedSignersFile ~/.ssh/allowed_signers
```
```
git clone -q --filter=blob:none --sparse git@github.com:eucalypsih/eucalypsih_rcrapsjava.git && sleep 1.1 && cd eucalypsih_rcrapsjava && sleep 1.1 && un="eucalypsih";ue="eucalypsih@gmail.com";git checkout main && sleep 1.1 && git config user.name "$un" && sleep 1.1 && git config user.email "$ue" && sleep 1.1 && git config gpg.format ssh && sleep 1.1 && git config user.signingkey ~/.ssh/id_rsa.pub && sleep 1.1 && git config commit.gpgsign true && sleep 1.1 && git config gpg.ssh.allowedSignersFile ~/.ssh/allowed_signers
```
